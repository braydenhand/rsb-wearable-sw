...
if page == "Home":
    ...
    st.write("This dashboard displays real-time sensor data from boxing vests.")
    st.write("Use the navigation on the left to view vest data or change settings.")
    
    # Display count of active vests
    vests = get_all_vests()
    active_vests = [v for v in vests if v.get("is_active", False)]
    
    st.metric("Active Vests", len(active_vests))
    
    # Show a preview of available vests
    if vests:
        st.subheader("Available Vests")
...
# Vest Dashboard Page
    ...
    if not vests:
        st.warning("No vests found. Please check your API connection.")
    else:
        # If not in detailed view, show all vests
        if "selected_vest" not in st.session_state:
            # Layout for vest cards
            vest_cards = []
            for i, vest in enumerate(vests):
...
# Settings Page
    ...
    if st.button("Save Settings"):
        # Update the API URL (in a real app, you'd save this to a config file or database)
